...
class ArrayBackedNetworkBuffer : public NetworkBuffer {
public:
    ArrayBackedNetworkBuffer() :
    ...
    void write(const uint32_t& val) override {
     ...
     * buffer
     */
    void write(const uint8_t* const buf, std::size_t numBytes) override {
     ...
     */
    uint8_t* read(std::size_t numBytes) override {
     ...
     * to next
     */
    const uint8_t* const getBuffer() const override {
        return _head;
    }
